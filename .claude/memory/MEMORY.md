...
- [A concurrent session designed the same tool](concurrent-session-designed-the-same-tool-2026-09-17.md) — two live sessions on one checkout, 90 min apart, same spec; check `git reflog` + worktree list BEFORE brainstorming, and never `checkout -b` on the shared checkout
- [Worktree session: the harness guard and the 8.3 venv path](worktree-session-harness-guard-2026-09-17.md) — quoted paths, `cd`, `-C` refused; use `/c/Users/EE-LT-11030/CLAUDE~4/EMB-Bot/digitizer/.venv/Scripts/python.exe`; preview_start reads the MAIN launch.json
