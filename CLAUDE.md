...
1. **DST axis bug — and it is a MIRROR, not "a quarter turn".** EMB-Bot's own DST codec (`src/dst.js` / `src/dstimport.js`) disagrees with the Tajima/pyembroidery standard. It round-trips correctly with itself and is wrong elsewhere in both directions. **This entry said "transposed" and the Studio told customers to rotate; rendered 2026-09-07, a standard reader sees the design a quarter turn round AND mirror-imaged — letters backwards — which no rotation repairs.** A swapped bounding box fits a turn and a mirror equally, and only a picture separates them: when a claim is about ORIENTATION, render it. **Import half FIXED 2026-09-07** — the lane reads `EMB.decodeDSTStandard`, so a third-party `.dst` lands correct and PES/EXP/JEF re-export identity/rms 0; `decodeDST` and its round-trip tests are untouched because they pair with the writer. **The WRITER is still Kent's call** (it re-orients every DST EMB-Bot has written), and it now owns the leftover: EMB-Bot's own `.dst` read back in is the file that comes in mirrored. Treat browser DST as EMB-Bot-internal only; the Python digitizer service's `/export` (pyembroidery convention) is the trustworthy path for anything leaving this app. Full evidence trail: `dst-codec-axis-discrepancy` in memory, DOCTRINE 2026-09-07, `test/dstimport.test.js`'s signed-area test.
