...
22. **Small curves sew as polygons — FIX BUILT, DEFAULT OFF, Kent's flip** (`curve_turn_deg`; None =
    ...
    midpoint, floored at one pixel. Fremont's counter **9 → 33 vertices, 47° → 17°**, inner rail σ 0.038 → 0.026 mm; off md5-identical. Near-floor lettering (ribbon width within 20% of the 0.5 mm minimum cross) is not refined, Kent's ruling — judged per RING, holes included, because a letter is also its background's hole and stage 5 reshapes it against that hole (review of #328: shell-only gating still dropped it to fill; per-ring keeps 28/16 of 28/16 satin penetrations ON vs OFF). **The flip re-pins curved goldens; held.** *(measured 2026-09-03 — `docs/round-curves-2026-09-03.md`)*
