...
4. **We trim far more than the professional — 3.1x the trim breaks on a
   ...
   **An open assumption under the whole 3.1x, named 2026-09-13:** this corpus was decoded from the pro's machine FILES, so it records what the FILE says — whether it also records what the MACHINE did is unestablished. A verified trade-knowledge pass put TEN machine-side trim-threshold claims to adversarial vote (Brother PR1050X jump-count, Melco's consecutive-jump count and default, Wilcom's "Maximum Jump") and **all ten were refuted, most unanimously**; no Tajima, Barudan or SWF doc was reached at all. One unvoted line survived inside another claim's verification — a machine trims only connectors longer than the length set ON THE MACHINE, whatever the file requests — which would make the observed cut/float split a joint product of file and machine setting. Plausible, mechanically; **not citable, and not currently assumed either way**. Separately and in our favour: vendors do NOT publish a distance rule — Hatch, *"if the connecting run is hidden beneath another object, it is more efficient to use a connecting run rather than a trim and tie-off"*, and a three-state per-object override whose Off/Always states bypass the mm value entirely; Embird publishes no threshold at all, only sew ORDER plus same-colour and visibility. **So "no single threshold reproduces this pro" is the expected shape of the published model, not an anomaly** — but note the scope mismatch: those rules govern needle-DOWN connecting runs and this corpus is needle-UP moves, so they constrain `chain_links`' routing, not whether a jump may go uncut. *(`docs/trade-knowledge-2026-09-13.md` §1, §6)*    *(2026-08-15/18/21, corrected 2026-09-06 — `docs/fragmentation-attribution-2026-08-18.md`, scope-history 09-06)*
