...
The measurement behind `tools/legibility.py` (PR 1 of
`docs/superpowers/plans/2026-09-08-real-logo-lane-and-thin-strokes.md`) and,
since 2026-09-10, behind preflight's `LETTERING_ILLEGIBLE` check
(`cfg.legibility_check`; quality review 2026-09-08 item 11, plan
`docs/superpowers/plans/2026-09-10-legibility-yardstick.md`). It moved here
so the yardstick and the instrument are the same code: `dropped_elements`
reads 0.2% on Hotel Fremont while Kent calls its tagline "completely lost",
because the instrument sees sewn-but-illegible as covered, and nothing in
the grade read what the thread SAYS.

For every text cluster stage 4 tagged (`meta["text_cluster_id"]`,
`textcluster.detect_text_clusters`), `measure` crops the same millimetres
out of the prepped artwork and out of `stitchviz.render_design`'s thread
render, runs tesseract on both, and reports the normalised edit similarity
between the two readings — 1.0 when the thread says what the art says, 0.0
when the art reads and the thread does not. The design-level figure is
weighted by the length of the artwork reading, so a cluster tesseract
cannot read on the ART side contributes nothing rather than a false zero.
When a design has no text cluster, the whole artwork and the whole render
are read once, as one row.

Its limits are measured and stand (scope-history 2026-09-08): OCR is a
LOWER bound on lettering loss — Fremont's THE reads 1.00 on a render that
shows T H C, because tesseract's word model fills the arm in, and
`tools/thin_strokes.py` sees that arm at 74% recall; a tagline that is not a
text cluster is invisible to it; Becker's art at 1.46 px/mm cannot be read
at all and contributes nothing. Gate 4: a per-cluster edit similarity is a
distance, not an agreement rate, and no chance floor is claimed for it.

Needs the `tesseract` binary (`tesseract_available`). CI installs it; the
Studio's Convert-to-text pass already depends on it; a box without it gets
the same skip the OCR tests take (`tests/conftest.py`, `requires_tesseract`).
"""
...
from .stitchviz import UNITS_PER_MM, _bounds, render_design
from .textcluster import _one_tesseract_thread
...
def _plan_frame(p: Prep) -> tuple[float, float, float]:
    ...
    (px - centre of the art bbox) / px_per_mm, y down. The same three lines
    as `tools/thin_strokes._plan_frame`, kept here so the package never
    imports a tool."""
...
# And an ART reading shorter than this is not a truth either: tesseract
# returns a single confident letter off a wordmark it cannot parse (Bridge
# Bar's 49 mm cluster reads "X" at 77, gaulke's 5.8 mm row "A" at 80, the
# screenshot's 2.5 mm row "I" at 60 — measured 2026-09-10), and a one-letter
# truth compared with a garbage render reading scores 0.00 for a loss nobody
# can see. Three letters is the shortest word the corpus carries on a row
# that reads cleanly on both sides (Fremont's THE, 96 / 83).
ART_MIN_LETTERS = 3
...
def read_pair(art_imgs, render_imgs,
    ...
    return {"art_text": a_txt, "art_conf": round(max(a_conf, 0.0), 1),
            ...
            "readable": len(normalise(a_txt)) >= ART_MIN_LETTERS and a_conf >= ART_CONF_MIN}
...
def measure(p: Prep, result, plan, px_per_mm: float = RENDER_PX_PER_MM,
    ...
    # Which shapes the plan actually sews. A cluster none of whose members
    # sews (enclosed letter bodies left as fabric, a shape the vectorizer
    # dropped) is reported with `sewn: False` and left to the warnings that
    # own unsewn shapes; the legibility FINDING judges sewn lettering only.
    sewn_ids = {r.shape_id for _b, r in plan.iter_runs()
                if getattr(r, "shape_id", None) and len(r.points)}
    if clusters:
        for cid, box in clusters:
            ...
            sewn = any(m.shape_id in sewn_ids for m in members[cid])
            ...
            rows.append({"cluster": cid, "height_mm": round(box[3] - box[1], 2),
                         "enclosed": enclosed, "sewn": sewn, **read,
        ...
        rows.append({"cluster": "(whole design)", "height_mm": round(box[3] - box[1], 2),
                     "enclosed": False, "sewn": bool(sewn_ids), **read,
                     "similarity": similarity(read["art_text"], read["render_text"])
                     if read["readable"] else None})
        if dump is not None:
