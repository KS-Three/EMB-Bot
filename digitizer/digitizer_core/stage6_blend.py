...
#
# `detect_ramp` asks "does this region's lightness fit a linear or radial
# ramp?", and on a real photograph the answer is no — measured on Kent's owl
# (2026-08-12), 11 of 11 blend-routed regions rejected on RAMP_R2_MIN, best r2
# 0.481 against a 0.5 floor, versus 0.994-0.999 for the committed synthetic
# ramps. That is not a tuning miss: an iris is a RING (dark pupil, amber band,
# darker rim) and an owl's breast is barred, and neither a plane nor a
# centroid-radial fit describes either. Lowering RAMP_R2_MIN would force a
# ramp model onto things that are not ramps.
#
# But "not a ramp" and "not worth shading" are different questions, and the
# tier was answering the second with the first. That same owl's body is a
# single 4200 mm2 region whose lightness spans 81 of the 100-point L* scale,
# sewn in ONE flat thread. Its tone has structure; it just isn't a slope.
#
# So this path drops the ramp MODEL and keeps the shade machinery: bucket the
# region's own pixels by lightness, take each bucket's polygon from the mask
# it actually occupies, and tatami-fill it. `stage6_streamline._shade_layers`
# already established the principle for the streamline tier (coverage-share
# maps driven by darkness, no r2 test anywhere); this is its tatami sibling,
# reusing `_choose_shade_count`/`_shade_lab_colors` verbatim rather than
# re-deriving the CIEDE2000 chart snap.
#
# A region qualifies only if BOTH hold:
#   * its extremes are far enough apart to want more than one thread, and
#   * it is big enough that splitting it still leaves each band real rows to
#     sew (row spacing multiplies by the shade count, exactly as the ramp
#     path's does).
# Below either floor, flat tatami remains correct and this path declines.
TONAL_MIN_EXTREMES_DELTAE = 18.0
# 2 * SHADE_STEP_DELTAE: the distance at which `_choose_shade_count` first
# asks for a 3rd shade. Below it the region wants 2 or fewer, and a 2-shade
# split of a photo region is noise, not structure.

# Each band must be able to hold at least this many fill rows at the
# post-split row spacing, or the "band" is a couple of disconnected stitches.
# Derived, not guessed: at n shades the row pitch is FILL_ROW_MM * n, so a
# band needs n * FILL_ROW_MM * TONAL_MIN_ROWS_PER_BAND mm of extent to be
# worth emitting. Checked against area rather than extent because a band's
# shape is unknown before it is cut.
TONAL_MIN_ROWS_PER_BAND = 3.0

# Smallest piece of a tonal band worth sewing on its own. A photo's tone map
# is speckled at the pixel scale, so thresholding it produces confetti
# alongside the real structure; anything under this is dropped back to the
# band around it rather than sewn as its own island. Same order as
# `machine.RUN_MIN_AREA_MM2`, which is the floor stage 4 already holds
# auto-digitized regions to.
TONAL_MIN_PIECE_MM2 = 2.0

# Morphological cleanup on each shade's raster mask before it becomes a
# polygon, in pixels. Closes single-pixel pinholes and shaves hairline
# peninsulas that would otherwise become unsewable slivers. Deliberately
# small: the goal is to remove quantisation artefacts, not to reshape the
# tone map.
TONAL_MASK_CLEAN_PX = 3
...
def _mask_to_polygons(mask: np.ndarray, sp: SourcePixels, x0: int, y0: int,
                      clip: Polygon) -> list[Polygon]:
    """A boolean raster mask -> shapely polygons in mm, clipped to `clip`.

    Contours come from `cv2.findContours` with RETR_CCOMP so a band's holes
    (a lighter core inside a darker ring — the iris case this whole path
    exists for) survive as interiors instead of being filled in.
    """
    if mask.size == 0 or not mask.any():
        return []
    m = mask.astype(np.uint8)
    if TONAL_MASK_CLEAN_PX > 1:
        k = np.ones((TONAL_MASK_CLEAN_PX, TONAL_MASK_CLEAN_PX), np.uint8)
    ...
    def to_mm_ring(c) -> list[tuple[float, float]]:
        return [sp.to_mm(int(px) + x0, int(py) + y0) for px, py in c[:, 0, :]]

    # RETR_CCOMP's hierarchy is exactly two levels: outer boundaries at the
    # top, their holes as children. hierarchy[0][i] = [next, prev, child, parent].
    out: list[Polygon] = []
    for i, contour in enumerate(contours):
        if hierarchy[0][i][3] != -1:      # a hole; consumed by its parent below
            continue
        if len(contour) < 3:
            continue
        holes = []
        ...
        while child != -1:
            if len(contours[child]) >= 3:
                holes.append(to_mm_ring(contours[child]))
            child = hierarchy[0][child][0]
        try:
            poly = Polygon(to_mm_ring(contour), holes)
            if not poly.is_valid:
                poly = poly.buffer(0)
        except Exception:  # noqa: BLE001 -- a degenerate ring is a skip, not a crash
            continue
        for part in _polygon_parts(poly):
            # Clipping to the region matters: the mask is rasterised on a
            # pixel grid, so a band's boundary can bulge a fraction of a
            # pixel past the shape's own vectorised outline. Without this,
            # stage 5's overlap resolution would see the blend bands poking
            # outside the region it was asked to fill.
            piece = part.intersection(clip)
            for final in _polygon_parts(piece if piece.is_valid else piece.buffer(0)):
                if final.area >= TONAL_MIN_PIECE_MM2:
                    out.append(final)
    return out


def tonal_bands(poly: Polygon, sp: SourcePixels, cfg
                ) -> tuple[list[tuple[Polygon, int]], list[np.ndarray], int]:
    """Lightness-bucketed shade bands for a region that is NOT a ramp.

    -> ([(band polygon, shade index)], shade Lab colours, shade count). An
    empty band list means this region did not qualify (or produced nothing
    sewable) and the caller should fall back to flat tatami.

    Bands are ordered light -> dark by shade index, matching
    `_shade_lab_colors`' own canonical positions, so the caller can sew them
    in whatever order it already sews ramp bands.
    """
    mm_x, mm_y, rgb, mask, _crop = _sample_pixels(poly, sp)
    if len(mm_x) < 12:
        return [], [], 0
    lab = rgb_to_lab(rgb)
    lightness = lab[:, 0]

    lo_i, hi_i = int(np.argmin(lightness)), int(np.argmax(lightness))
    extremes_delta_e = float(
        deltaE_ciede2000(lab[lo_i:lo_i + 1], lab[hi_i:hi_i + 1])[0]
    )
    if extremes_delta_e < TONAL_MIN_EXTREMES_DELTAE:
        return [], [], 0

    n = _choose_shade_count(extremes_delta_e)
    if poly.area < n * machine.FILL_ROW_MM * TONAL_MIN_ROWS_PER_BAND * n:
        # Area floor scales with n twice over: n bands each needing n-times-
        # wider row pitch. A 4200 mm2 owl body clears this comfortably at
        # n=5; a 15 mm2 iris does not, and stays one honest flat colour.
        return [], [], 0

    # `ts` is each sample's position on the same [0, 1] light->dark scale
    # `_shade_lab_colors` buckets on, so the colours it returns and the
    # bands cut below are two views of ONE partition.
    l_lo, l_hi = float(lightness.min()), float(lightness.max())
    if l_hi - l_lo < 1e-6:
        return [], [], 0
    ts = (lightness - l_lo) / (l_hi - l_lo)
    shade_labs = _shade_lab_colors(ts, lab, n)

    # Now cut the FULL raster (not the subsample) on the same thresholds.
    crop, full_mask, x0, y0 = _crop_and_mask(poly, sp)
    if crop.size == 0 or not full_mask.any():
        return [], [], 0
    flat_rgb = crop.reshape(-1, 3)
    full_l = rgb_to_lab(flat_rgb)[:, 0].reshape(crop.shape[:2])
    full_t = np.clip((full_l - l_lo) / (l_hi - l_lo), 0.0, 1.0)

    centers = np.array([i / (n - 1) for i in range(n)]) if n > 1 else np.array([0.5])
    nearest = np.argmin(np.abs(full_t[..., None] - centers[None, None, :]), axis=2)

    bands: list[tuple[Polygon, int]] = []
    for i in range(n):
        band_mask = full_mask & (nearest == i)
        for part in _mask_to_polygons(band_mask, sp, x0, y0, poly):
            bands.append((part, i))
    return bands, shade_labs, n


# --- Emission ------------------------------------------------------------------

def _tonal_band_fill(region: Region, source_pixels: SourcePixels, cfg
                     ) -> tuple[list[StitchRun], dict]:
    """Tonal-band decomposition -> (stitches, report), or ([], {}) if this
    region did not qualify. Mirrors the ramp path's emission exactly — same
    row-pitch multiplication, same shade-boundary jump marking, same report
    aggregation — so stage 7 cannot tell the two apart.
    """
    poly = region.polygon
    bands, shade_labs, n = tonal_bands(poly, source_pixels, cfg)
    if not bands:
        return [], {}
    ...
    # NORMAL row pitch, deliberately NOT the ramp path's `FILL_ROW_MM * n`.
    #
    # That multiplication is right for ramp bands and wrong here, because the
    # two band geometries are different. A ramp band is a contiguous STRIP
    # spanning the region's full width and 1/n of its length, so the strips
    # laid side by side re-tile the region once; widening the pitch by n
    # keeps the design's total stitch budget near a flat fill's.
    #
    # A tonal band is not a strip. It is the set of patches at one lightness,
    # scattered across the WHOLE region and interleaved with the other
    # shades' patches. Each band already covers only ~1/n of the area while
    # spanning the full extent, so multiplying the pitch on top of that
    # under-sews it a second time — measured on Kent's owl body: 6058
    # stitches flat, 1971 at `FILL_ROW_MM * n`, a third of the coverage and
    # visibly sparser. The same failure the earlier `streamline_mode:
    # "layered"` comparison recorded as "3,220 stitches = worse".
    #
    # The bands partition the region, so at normal pitch they sum back to a
    # flat fill's density — the same thread budget, spent on n shades
    # instead of one.
    ...
    report = {"too_thin": False, "jumps": 0, "empty": False,
              "blend_shades": n, "blend_reject": RAMP_OK, "blend_best_r2": 0.0,
              "blend_kind": "tonal"}
    # Sew dark shade first, same order `stage6_streamline._shade_layers`
    # hands its layers back in: the darker thread reads as the ground the
    # lighter ones sit on, and a light thread laid over a dark one covers
    # better than the reverse.
    for i in range(n - 1, -1, -1):
        this_layer: list[StitchRun] = []
        for part, shade_i in bands:
            if shade_i != i:
                continue
            runs, band_report = stitch_shape(
                part, f"{region.shape_id}-tone{i}", angle_deg=angle,
                row_mm=row_mm, stitch_mm=machine.FILL_STITCH_MM,
                underlay_style="none", trim_at_mm=machine.TRIM_AT_MM,
            )
            if runs and this_layer:
                # Two disconnected pieces of ONE shade: same-colour gap, so a
                # plain jump is right and there is no ring to bridge along.
                # Identical reasoning to the ramp path's parts-within-a-band
                # case, which this mirrors deliberately.
        ...
        if this_layer and all_runs:
            # Shade change: bridging would smear the previous thread across
            # the seam, so jump rather than travel. Same call the ramp path
            # documents at its own band boundaries.
    ...
    if not all_runs:
        # Every band cleaned away to nothing. Caller falls back to flat
        # tatami rather than this returning an empty design.
        return [], {}
...
def blend_fill(region: Region, source_pixels: SourcePixels, cfg
    ...
    if model is None and getattr(cfg, "blend_tonal_bands", False):
        # Not a ramp — but possibly still worth shading. See the
        # "Tonal bands" section above for why these are different questions.
        runs, report = _tonal_band_fill(region, source_pixels, cfg)
        if runs:
