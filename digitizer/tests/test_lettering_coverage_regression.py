...
def test_lettering_coverage_has_not_regressed_since_the_rail_change():
    ...
    **The sentence that used to sit here named the wrong cure, and it is kept
    ...
    and a cure moves thread back onto the letter's edge rather than adding
    coverage. Do not give back 768de79e's jitter win buying it — check
    `tools/rail_edge.py` alongside, and see the counter-trade guards below.
    """
