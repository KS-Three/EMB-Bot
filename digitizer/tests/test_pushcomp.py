...
# (`PipelineConfig.fill_travel_under_cover`, default ON by Kent's flip that
# day; stage6_fill's `_reorder_for_cover` and `travel_path(sewn=...)`): the
# fill's column order prefers a next column reachable over ground not yet
# sewn, and a bridge that would lie on finished fill is routed through unsewn
# ground. On this fixture that is FOUR fewer travel penetrations, 2166 ->
# 2162; no trim or jump moved. Earned the same way as the 2026-08-15 pin: a
# worktree at origin/main (f4009a6; digitizer_core identical to baf702c)
# reproduces ("05a59e2bc2a524087c4f", 2166, 7073) byte-for-byte on this
# machine, so what moved is the engine. `towel` is unchanged by the same
