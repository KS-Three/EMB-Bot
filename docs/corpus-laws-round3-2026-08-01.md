...
| 26 | `fabrics.fill_underlay` `edge_lattice` → **`edge_run`** | affects pique_knit, jersey_tee; leave the pile presets' heavier styles for their own sew-out | **shipped 2026-08-05 in Python; the browser half landed 2026-09-07** — `digitizer_core/fabrics.py` took it that day, `src/fabrics.js` did not, so for a month the two engines ran different underlay on left_chest, beanie and sleeve (+1.4% to +5.7% browser stitches). Guarded now by `tests/test_fabric_wire.py`. |
