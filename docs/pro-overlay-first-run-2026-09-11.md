...
52 flagged rows. `S78e6cd01` is the patch's own background tatami (fill/fill, no tier mismatch — flagged only on width/direction). Four named elements identified from the artwork and the region bounds (`ours_regions.json`'s `bounds` field, in the same registered mm frame `--crop` takes):
...
The remaining flagged rows (`S234804ac`, `S1800e700`, `S942bc7ba`, `Sf4300e5b`) are the plain black rule lines flanking "THE" and under "HOTEL FREMONT" — not one of Kent's previously-reviewed named elements, no separate crop made.
...
4. **Fremont lettering column width (the pro 0.82-0.90 mm where we sew bean runs) — REFUTED.** The measured pro column width on "HOTEL FREMONT"'s own satin-tier letters runs 1.30-1.50 mm p50, not 0.82-0.90 mm — e.g. `Sec0656d0` (0.80 / 1.40), `S9c0c9dc7` (0.80 / 1.40), `Sf629d513` (0.80 / 1.40), `Sf0dec763` (0.80 / 1.40), `Sbb420d8d` (0.77 / 1.50). Ours runs narrower than the pro here (0.70-0.80 mm), not wider as a "bean run" would read — only 3 of 16 lettering-band regions (`S3d684c8d`, `Sf979bc42`, `S10d3c3b0`) fall back to a `run` or `fill` tier on our side at all, and none of those three measure a pro width in the 0.82-0.90 mm range either (1.30-1.50 mm again). The 0.82-0.90 mm figure is not visible anywhere in this catalogue's lettering rows.
