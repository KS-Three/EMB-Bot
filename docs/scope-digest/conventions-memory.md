...
- Windows-specific: golden divergence is per-fixture, not per-platform — Kent's Windows fails `enthusiast_logo` ×2 + `pushcomp` towel while `logo_alpha` ×2 PASS there (a local `logo_alpha` failure is a real regression); CI deselects a DIFFERENT trio (`logo_alpha` ×2 + towel); divergence is one contour, 0.3208 vs 0.3784 mm². Judge by "same failure set before and after" against MASTER_SCOPE § Gotchas' matrix; expect 3 failed locally with `.[service]` — the 5 real-read OCR tests skip without `tesseract` since 2026-08-17 (never on CI, where a missing binary fails loud) (memory/windows-goldens-fail-locally.md; COOKBOOK.md § Running things).
