...
- **Repo visibility: CLAUDE.md says "This repo is private — it holds competitor teardowns, a legal brief, and the defect list. Keep it that way." `pull-corpus` SKILL.md and `digitizer/testdata/inbox/README.md` say "`KS-Three/EMB-Bot` is a PUBLIC repository (verified 2026-08-14 via the GitHub API — `visibility: public`)" and route client/third-party art into it.** These cannot both hold and the wrong one is irreversible in git history. Resolve before any asset pull.
