...
- **"Zero behaviour change outside that pattern"** (`photo-palette-overflow § Effect on other designs`) vs `MASTER_SCOPE.md § Waiting on Kent #5`: `split_tonal_regions` routinely pushes the palette to the `max_colors + PALETTE_OVERFLOW_K` ceiling, so the overflow path becomes ordinary the moment that flag flips (parked until the sew-out).
