...
widened lettering keeps its growth over its ground; and the layer unions the
stage plans against carry a widened member's sewn COLUMN, not its artwork,
so a ground whose thread sews AFTER the lettering (largest-area-first order
does that when the lettering's thread holds the biggest shape) is clipped
to the column instead of burying 79% of it — the review's find. Fremont: 8
of 10 widened glyphs sew **169 columns at a 0.91 mm median** (the pro:
0.82–0.90), routed legibility 0.551 → 0.577, stitches 16,006 → 15,823 — and
the crops show the 1.0 mm column filling counters at a 2.2 mm cap height.
ENTHUSIAST: a 1.0 mm column in a 1.6 mm glyph is a smear (p10 0.16 mm;
legibility 1.00 → 0.72), so the floor needs a glyph-height gate before it
ships on — gate 1, Kent's. Tests: `tests/test_widened_lettering_tier.py`
(6; the panel test mutation-proved against the stage-5 exemption, the
sew-order test against the artwork clip). Table and crops: scope-history
09-09, second entry; `docs/renders/column-tier-2026-09-09/`.
...
| 4, the tier rule | **BUILT 2026-09-09** — a widened door-1 member takes the column route in stage 7 (exempt from the sub-floor run routing; classified and sewn on its compensated polygon; bean-run fallback) AND in stage 5 (keeps its growth over the ground it was vectorized as a hole in — the half the negative did not name). Fremont: 8 of 10 widened glyphs sew 169 columns at 0.91 mm; ENTHUSIAST's 1.6 mm subline smears, legibility 1.00 → 0.72 — a glyph-height gate is the open question (§4d) | ~130 + 6 tests | G1 on the number, unchanged; the flag stays off |
