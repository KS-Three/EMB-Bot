...
**A held ruling to carry over (Kent's call, 2026-09-18).** A throwaway preview of this page — `https://claude.ai/artifact/6Rc8urLWN1fXc8oh7xVtPg`, real renders of three NON-corpus fixtures with EXAMPLE picks — proved the `db` write path and that a republish leaves notes alone. While trying it Kent left a real `rulings/<arm>` document there, written **without a render of that arm in front of him**. He chose to HOLD it for the sitting rather than record it in the repo, so that it cannot be read as a verdict or steer his blind picks. At the real publish: `ArtifactData list rulings` on the preview URL, copy any document not marked `SEEDED BY CLAUDE` into the real artifact's `rulings/`, and only then record it — beside what his eye actually picked for that arm. Its content is deliberately not restated here.
