...
From `digitizer/eye_pairs_out/` (yardstick §3.4), or a `docs/eye-pairs-<date>/`
copy for the three small files:
...
garment per fixture come from `tools.thin_strokes.corpus_cases()`, the
yardstick's own source; a fixture the table does not know shows blanks, not
an error.
...
list — never summed into a percentage.

## 4. The page

One static HTML document, no framework, no CDN dependency, tokens on `:root`,
dark mode honoured, phone width usable.
...
- the chips row: metric names, green where the metric points the way he
  picked, red where it points the other way; a chip's values show on hover;
  the exit-clause rows are marked;
...
- W/L/T tally per arm, with identical skips counted from `arms.json`;
