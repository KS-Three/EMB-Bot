...
// EVERY byte below was generated by `pystitch.DstWriter.encode_record`, not
// derived by hand: X lives in the LOW nibble and Y in the HIGH one. They
// pinned the mirror image of that until 2026-09-08 (X high, Y low), which is
// the transposition docs/dst-axis-verdict-2026-07-31.md identified and which
// no reader but our own agreed with. Verified 10/10 byte-identical to
// pystitch across these deltas. (Our dy is pystitch's y negated -- a frame
// choice, not a disagreement: pystitch's writer does `y = -y` on entry.)
...
test("dx=1", () => assert.deepStrictEqual(dst.encodeRecord(1,0,"stitch"), b(0x01,0x00,0x03)));
test("dx=-1", () => assert.deepStrictEqual(dst.encodeRecord(-1,0,"stitch"), b(0x02,0x00,0x03)));
test("dy=1", () => assert.deepStrictEqual(dst.encodeRecord(0,1,"stitch"), b(0x80,0x00,0x03)));
test("dx=9", () => assert.deepStrictEqual(dst.encodeRecord(9,0,"stitch"), b(0x04,0x00,0x03)));
test("dx=13 = 9+3+1", () => assert.deepStrictEqual(dst.encodeRecord(13,0,"stitch"), b(0x05,0x01,0x03)));
test("dx=121 max", () => assert.deepStrictEqual(dst.encodeRecord(121,0,"stitch"), b(0x05,0x05,0x07)));
test("dx=2 = +3-1", () => assert.deepStrictEqual(dst.encodeRecord(2,0,"stitch"), b(0x02,0x01,0x03)));
test("dx=5 = +9-3-1", () => assert.deepStrictEqual(dst.encodeRecord(5,0,"stitch"), b(0x06,0x02,0x03)));
test("dy=2 = +3-1", () => assert.deepStrictEqual(dst.encodeRecord(0,2,"stitch"), b(0x40,0x80,0x03)));
...
// Swapped 2026-09-08 with the codec itself: X is the LOW nibble, Y the HIGH.
const X_DECODE = [
