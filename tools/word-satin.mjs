...
const { runs, travels, bbox } = SF.layoutText(font, TEXT, { emMm: EMMM, pxPerMm: 10, spacingMm: 0.4, pullCompMm: 0.2 });
...
for (const [a, b] of travels) line(TX(a.x), TY(a.y), TX(b.x), TY(b.y), 245, 205, 170);
...
console.log(JSON.stringify({ text: TEXT, runs: runs.length, travels: travels.length, widthMm: +((bbox.x1 - bbox.x0) / 10).toFixed(1), heightMm: +((bbox.y1 - bbox.y0) / 10).toFixed(1), out: OUT }));
